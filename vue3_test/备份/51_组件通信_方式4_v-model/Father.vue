<template>
  <div class="father">
    ...
    <!-- 并且上述的语句中的（<HTMLInputElement>）的结构为断言的结构（ts内容），表示解释当前的内容“一定”为HTML中的输入
    元素标签对象 -->
...
<!-- 需要注意的是需要实现组件通信的话，父组件中的子组件标签的形式可以是：
<AtguiguInput v-model="username"/>
<AtguiguInput :modelvalue="username" @update:modelvalue="username = $event"/>
但是子组件中则必须是<input :value="username" @input="emit("update:modelValue",(<HTMLInputElement>$event.target).value)"/>的形式
原因是父组件中的第二种形式是第一种形式的固定的底层写法;而子组件中的input如果要向父组件中传递形参中的信息的话则需要使用到emit方法,则必须是这样的形式

注意上述的为组件赋值为v-model的含义和为input标签赋值为v-model的含义之间的区别在于：
<input v-model="username"/>
<input :modelvalue="username" @input="username = (<HTMLInputElement>$event.target).value"/> -->